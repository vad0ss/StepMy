...
public class GroceryItemFactory {
    ...
    private List<String> categoryNames = List.of("DAIRY", "VEGETABLE", "FRUIT", "BAKERY", "MEAT", "BEVERAGE");
    private Map<Category, List<String>> map = Map.of(
    ...
    public GroceryItem next() {
        Category category = Category.valueOf(categoryNames.get(random.nextInt(0, 2)));
        List<String> names = map.get(category);
        String name = names.get(random.nextInt(0,2));

        return new GroceryItem(name, category, random.nextDouble(1,100), random.nextBoolean());
    }
